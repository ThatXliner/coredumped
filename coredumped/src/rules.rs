            ...
            rules: vec![
                ...
                Rule {
                    ...
                    source_lines: &[
                        "(defrule rage-impact",
                        "  {:phase :enemy-ai :cost :tick}",
                        "  ;; Collision payload is copied into a 64-byte buffer.",
                        //"  ;; BUG: charged impacts can supply larger payloads.",
                        //"  ;; Adjacent memory: registry write-protect flag.",
                        "  ;; Try a charged strike: (do-attack :east 13)",
                        "  (let impact-force (last-impact-force *self*)",
                        "    (do",
                        "      (when (> impact-force 12)",
                        "        (let log (open-registry :spawn-log)",
                        "          (copy-bytes! (bytes 64) impact.payload)",
                        "          (log :write :shockwave impact-force)))",
                        "      (if (adjacent? *self* *player*)",
